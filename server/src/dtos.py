...
class Party(BaseModel):
    ...
    bank_accounts: list[BankAccount] = Field(default_factory=list, description="Bank accounts associated with this party")


class SearchRequest(BaseModel):
    invoice_number: str = Field(min_length=0, description="Invoice number")
    invoice_date: str = Field(min_length=0, description="Invoice date, preferably YYYY-MM-DD")
    currency: str = Field(min_length=0, description="Invoice currency")
